...
class VentaServiceImplTest {
    ...
    void testSave_VentaSinDetalles_GuardaDirecto() {
        ...
        assertNotNull(resultado);
        verify(inventarioService, never()).consultarStockDisponible(anyLong());
    ...
    @Test
    void testSave_CatalogoCaidoEnSalida_noGuardaVenta() {
        when(inventarioService.consultarStockDisponible(1L)).thenReturn(20);
        doThrow(new CatalogoServiceUnavailableException("Catálogo-inventario no disponible"))
                .when(inventarioService).registrarSalida(1L, 5);

        assertThrows(CatalogoServiceUnavailableException.class, () -> ventaService.save(venta));
